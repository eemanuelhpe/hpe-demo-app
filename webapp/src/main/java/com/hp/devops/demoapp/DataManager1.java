...
public class DataManager1 {
	...
	static void init(ServletContext servletContext) {
		...
		if (servletContext != null) {
			DataManager1.servletContext = servletContext;
			DataManager1.loadData();
		} else {
	...
	static void loadData() {
		System.out.println("some chang.. e");
		try {
	...
	static void upVoteBand(int id) throws Exception {
		if (!initialized) throw new Exception("service not initialized");
		Band band = DataManager1.getBand(id);
		if (band != null) {
			synchronized (band) {
				band.votes++;
			}
